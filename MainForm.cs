...
namespace FillFlex
{
    public partial class MainForm : Form
    {
        ...
        private bool modbusconnection = false;
        private static logger Logger = new logger(GVL.appPath + "/FillFlexInit/Logs.csv");
...
//  ===================================  1 sec TIMER =================================== 
        ...
        private void TimeTick_Tick(object sender, EventArgs e)
        {
            ...
            if (GVL.isAdmin && !GVL.fixLogin) {
                ...
                GVL.fixLogin = true;
                syscmdval.Visible = true;
            }
            if (!GVL.isAdmin && GVL.fixLogin)
            {
                LogMessage("Виконано вихід з системи", 2);
                GVL.fixLogin = false;
                syscmdval.Visible = false;
            }
            ShowTrend();
        }
...
//  ===================================  START UP  =================================== 
        ...
        private void StartUp() {
            ...
            */
            ...
            GVL.FillMaxLimit = int.Parse(GVL.settings[14]);


            GVL.useTrends = int.Parse(GVL.settings[15]);
            if (GVL.useTrends > 0)
            {
                this.TrendsBtn.Visible = true;
                this.TrendsTimer.Enabled = true;
                this.TrendsTimer.Interval = int.Parse(GVL.settings[16]);
                sett15.Text = "так";
            }
 ...
 //           modbustcp.plcInstance.LogFileFilename = GVL.appPath + "/FillFlexInit/modbuslogs.txt";
            // --- meh indexes ---
            ...
            LoadEvents();
            dbconnection = AppDb.OpenDb();
            if (dbconnection)
            {
                ShowDataGridView();
                TimeTick.Enabled = true;
                modbusconnection = modbustcp.OpenConn();
                if (modbusconnection)
                {
                    ...
                    DialogResult res = MessageBox.Show("Не можу підключитися до ПЛК...", "Зв'язок", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    CommFail();
                    //Application.Exit();
                    //this.Close();
                    //this.Dispose();
                ...
                DialogResult res = MessageBox.Show("Не можу підключитися до бази даних...", "Вихід", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Application.Exit();
                this.Close();
                this.Dispose();        
            }
...
//  ===================================  EVENTS LISTBOX  ===================================
        ...
        private void LoadEvents() {
            ...
            using (StreamReader reader = new StreamReader(GVL.appPath + "/FillFlexInit/Logs.csv", System.Text.Encoding.Default))
            {
                while (true) {
...
//  ===================================  PLC CONTROL  ===================================
        ...
        private void CommFail() {
            ...
            ConnInd.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/RedConn.png");
        ...
        private void CommRestore()
        {
            ...
            ConnInd.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/GreenConn.png");
            Anim();
        }
...
// ------------- valve ------------- 
            ...
            {
                V1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/VH_red.png");
            }
            else if (!modbustcp.GetBit(12, 0) && !modbustcp.GetBit(12, 1)) {
                V1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/VH_yellow.png");
            }
            else if (modbustcp.GetBit(12, 0))
            {
                V1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/VH_green.png");
            }
            else {
                V1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/VH_grey.png");
            }
// ------------- pump -------------
            if (modbustcp.GetBit(0, 2) || modbustcp.GetBit(0, 3) || modbustcp.GetBit(25, 2))
            {
                P1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/PH_red.png");
            }
            else if (modbustcp.GetBit(0, 1) && !modbustcp.GetBit(0, 0))
            {
                P1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/PH_yellow.png");
            }
            else if (modbustcp.GetBit(0, 0))
            {
                P1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/PH_green.png");
            }
            else
            {
                P1pictbox.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/PH_grey.png");
            }
// ------------- task indicators -------------
...
// ------------- indicators ------------- 
            ...
            // flowmeter
                ...
                {
                    FlowMetInd.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/FlowMeterRed.png");
                }
                else
                {
                    FlowMetInd.Image = Image.FromFile(GVL.appPath + "/FillFlexInit/Images/FlowMeterGreen.png");
                }
            // sys triggers D25
...
//  ===================================  FUNCTIONS  ===================================
        ...
        private Image SetInd(int register, int bit) {
            if (bit > 15)
            {
                ...
                {
                    return Image.FromFile(GVL.appPath + "/FillFlexInit/Images/Ind_green.png");
                }
                else
                {
                    return Image.FromFile(GVL.appPath + "/FillFlexInit/Images/Ind_grey.png");
                }
            }
            else {
        ...
        private Image SetAlm(int register, int bit)
        {
            if (bit > 15)
            {
                if (modbustcp.GetBit(register + 1, bit - 16))
                {
                    return Image.FromFile(GVL.appPath + "/FillFlexInit/Images/Ind_red.png");
                }
                else
                {
                    return Image.FromFile(GVL.appPath + "/FillFlexInit/Images/Ind_grey.png");
        ...
        private Image SetWarn(int register, int bit)
        {
            if (bit > 15)
            {
                if (modbustcp.GetBit(register + 1, bit - 16))
                {
                    return Image.FromFile(GVL.appPath + "/FillFlexInit/Images/Ind_yellow.png");
                }
                else
                {
                    return Image.FromFile(GVL.appPath + "/FillFlexInit/Images/Ind_grey.png");
...
// =================================== SCRIPTS ===================================
        ...
        private void ShowProgress() {
            Single lastSetpoint = Single.Parse(lastSetp.Text);
            double Val = 0.0;
            if (lastSetpoint > 0.0)
            {
                Val = Math.Round((modbustcp.GetReal(5792) / lastSetpoint) * 100, 2);
            }
            if (Val > 100.0)
            {
                Val = 100.0;
        ...
        private void TrendSaveData() { // call from 1 sec. timer
            if (modbustcp.Connected())
            {
                Random rnd = new Random();
                double param0 = rnd.Next(0, 201);  // creates a number between 0 and 200
                double param1 = rnd.Next(0, 101);  // creates a number between 0 and 100
                double param2 = rnd.Next(0, 51);  // creates a number between 0 and 50 
                List<double> trendValues = new List<double>();
                trendValues.Add(param0);
                trendValues.Add(param1);
                trendValues.Add(param2);
                AppDb.TrendWrite(trendValues);
            }
        }

        private void TrendsBtn_Click(object sender, EventArgs e)
        {
            //List<List<string>>trendData = AppDb.getTrendData(50);
            List<List<string>> trendData = AppDb.getTrendData(AppDb.GetTrendDataCount());
            trendsForm TrendWind = new trendsForm(trendData, "Тренди параметрів");
            TrendWind.Show();
        }
